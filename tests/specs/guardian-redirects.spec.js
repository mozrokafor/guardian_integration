...
test.describe('guardian redirects', () => {
  ...
  test.describe('redirects to mozilla.org', () => {
    test(`Verify redirect for ${baseUrl}, C1538764`, async ({ page }) => {
      ...
      await page.goto(`${baseUrl}/products/vpn`, { waitUntil: 'networkidle' })
      verifyUrl(page, `${baseUrl}/en-US/products/vpn/`)
    })

    // test(`Verify redirect for ${baseUrl}/r/vpn/invite, C1539666`, async ({ page }) => {
    //   if (process.env.TEST_ENV === 'prod') {
    //     await page.goto(`${baseUrl}/vpn/invite`, { waitUntil: 'networkidle' })
    //     console.log(`${baseUrl}/vpn/invite`)
    //     verifyUrl(page, `${baseUrl}/en-US/vpn/invite`)
    //   }
    // })
