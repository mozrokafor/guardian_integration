...
test.describe('guardian localization, C1538760', () => {
  ...
  for (const language of supportedLanguages) {
    ...
    test.describe(
      `${language.timezoneId} - https://vpn.mozilla.org/vpn/client/login/success`,
      () => {
        test.use({ locale: language.locale, timezoneId: language.timezoneId })
        test.beforeEach(async ({ page }) => {
          await page.goto('https://vpn.mozilla.org/vpn/client/login/success', {
            waitUntil: 'networkidle',
          })
        })

        test(`Verify availability in ${language.timezoneId} for for /login/success`, async ({
          page,
        }) => {
    ...
    test.describe(`${language.timezoneId} - https://vpn.mozilla.org/vpn/client/login/error`, () => {
      test.use({ locale: language.locale, timezoneId: language.timezoneId })
      test.beforeEach(async ({ page }) => {
        await page.goto('https://vpn.mozilla.org/vpn/client/login/error', {
          waitUntil: 'networkidle',
        })
      })

      test.only(`Verify availability in ${language.timezoneId} for /login/error`, async ({
        page,
      }) => {
